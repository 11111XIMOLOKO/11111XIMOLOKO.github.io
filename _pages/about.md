---
permalink: /
title: "About"
author_profile: true
redirect_from:
  - /about/
  - /about.html
---

I'm an undergraduate at Jiangxi Normal University, majoring in Computer Science and Technology, 2023–2027 (expected). My research focuses on **diffusion-based image restoration** and **industrial anomaly detection**. I'm currently building a full-stack defect repair pipeline using Stable Diffusion, ControlNet, and DDIM Blending — reading source code to understand every layer, not calling an API. Actively seeking MS/PhD opportunities starting Fall 2027.

## Ranking

* Comprehensive: **2/91** (top 2.2%)
* GPA: **4/91** (top 4.4%)
* CET-6: **479** · Graduating June 2027

## Scholarships

<p>🎓 <strong>First-class</strong> Comprehensive Scholarship, 2023–2024</p>
<p>🎓 <strong>First-class</strong> Comprehensive Scholarship, 2024–2025</p>

## Campus Honors

<p>🏅 University-level Merit Student, 2023–2024</p>
<p>🏅 University-level Merit Student, 2024–2025</p>
<p>🏅 Civilized University Student, 2024</p>
<p>🏅 Outstanding Student Leader, 2025</p>
<p>🏅 Outstanding Communist Youth League Member, 2025</p>
